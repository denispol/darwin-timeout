...
mod tests {
    ...
    fn test_throttle_state_has_start_fields() {
        ...
        assert!(
            state.start_cpu_ns > 0,
            "should have positive initial CPU time"
        );
        assert_eq!(
            state.start_cpu_ns, state.last_cpu_ns,
            "start and last should match initially"
        );
        assert_eq!(
            state.start_wall_ns, state.last_wall_ns,
            "start and last should match initially"
        );
    }
    ...
    fn test_integral_control_under_budget_stays_running() {
        ...
        assert!(
            total_cpu_at_budget <= budget_ns,
            "at budget should still run"
        );
    }
    ...
    fn test_integral_control_convergence() {
        ...
        assert!(
            cpu_at_target <= budget_10s,
            "at target should be allowed to run"
        );

        /* if process used 5.001s CPU, it's over target and stops */
        let cpu_over_target = 5_001_000_000u64;
        assert!(
            cpu_over_target > budget_10s,
            "over target should be stopped"
        );
    }
}
