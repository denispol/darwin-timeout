...
#[test]
fn test_completions_includes_all_options() {
    /*
     * Verify completions include every long option we support.
     * If we add a new flag, this test reminds us to check it appears.
     */
    timeout_cmd()
        .args(["--completions", "bash"])
        .assert()
        ...
        .stdout(predicate::str::contains("--timeout-exit-code"))
        .stdout(predicate::str::contains("--on-timeout"))
        .stdout(predicate::str::contains("--on-timeout-limit"))
        .stdout(predicate::str::contains("--completions"));
}

#[test]
fn test_completions_includes_short_flags() {
    /*
     * Short flags are easy to forget in completion scripts.
     * Verify all our short options show up in the opts list.
     *
     * The bash opts string looks like: opts="-s -k -p -f -v -q -h -V ..."
     * We check for the flags with surrounding context to avoid false positives.
     */
    timeout_cmd()
        .args(["--completions", "bash"])
        .assert()
        .success()
        .stdout(predicate::str::contains("-s -k"))
...
#[test]
fn test_completions_bash_valid_syntax() {
    /*
     * Generated bash completions must be syntactically valid.
     * Pipe through `bash -n` which parses without executing.
     */
    let output = timeout_cmd()
        .args(["--completions", "bash"])
        .output()
    ...
    assert!(output.status.success(), "completion generation failed");
    ...
    let syntax_check = std::process::Command::new("bash")
        ...
        .spawn()
        .expect("bash not found - skip this test if bash unavailable");
    ...
    let result = syntax_check.wait_with_output().expect("bash failed");
    assert!(
        result.status.success(),
        "bash completion script has syntax errors: {}",
        String::from_utf8_lossy(&result.stderr)
    );
}

#[test]
fn test_completions_zsh_valid_syntax() {
    /*
     * Generated zsh completions must be syntactically valid.
     * Pipe through `zsh -n` which parses without executing.
     */
    let output = timeout_cmd()
        .args(["--completions", "zsh"])
        .output()
    ...
    let zsh_check = std::process::Command::new("which").arg("zsh").output();

    if zsh_check.is_err() || !zsh_check.unwrap().status.success() {
        eprintln!("zsh not found, skipping syntax validation");
        return;
    }
    ...
    let result = syntax_check.wait_with_output().expect("zsh failed");
    assert!(
        result.status.success(),
        "zsh completion script has syntax errors: {}",
        String::from_utf8_lossy(&result.stderr)
    );
}

#[test]
fn test_completions_fish_valid_syntax() {
    /*
     * Generated fish completions must be syntactically valid.
     * Use `fish --no-execute` to parse without running.
     */
    let output = timeout_cmd()
        .args(["--completions", "fish"])
        .output()
    ...
    let fish_check = std::process::Command::new("which").arg("fish").output();

    if fish_check.is_err() || !fish_check.unwrap().status.success() {
        eprintln!("fish not found, skipping syntax validation");
        return;
    }

    let syntax_check = std::process::Command::new("fish")
        .arg("--no-execute")
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped())
    ...
    syntax_check
        ...
        .unwrap()
        .write_all(&output.stdout)
        .expect("failed to write to fish stdin");

    let result = syntax_check.wait_with_output().expect("fish failed");
    assert!(
        result.status.success(),
        "fish completion script has syntax errors: {}",
        String::from_utf8_lossy(&result.stderr)
    );
}
